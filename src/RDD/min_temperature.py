...
f_min = lines.filter(lambda x: 'TMIN' in x[2])
f_mins = f_min.map(lambda x: (x[0], float(x[3])*0.1))
min_temps = f_mins.reduceByKey(lambda x,y: min(x,y))
...
t_mins = lines.filter(lambda x: 'TMIN' in x[1])
t_min = t_mins.map(lambda x: (x[0], x[2]))
min_temps = t_min.reduceByKey(lambda x,y: min(x,y))
for i in min_temps.collect():
